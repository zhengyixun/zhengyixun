...
$(window).ready(function(){
	...
	let Want=$('.Want')
	let flag
	let heights=Want[now1].offsetHeight
	function move1(){
		next1++
		...
		if(next1==Want.length){
			next=0
		}
		Want.eq(next1).css('top',`${-heights}px`)
		Want.eq(now1).animate({'top':`${heights}px`})
		Want.eq(next1).animate({'top':0},function(){flag=true})
		now1=next1
	}
	function move11(){
		next1--
